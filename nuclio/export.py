...
class NuclioExporter(Exporter):
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        seen_function_name = ""
        function_buffers = {
            function_name: {"closed": False, "codes": [],},
            "": {"closed": False, "codes": [],},
        }
        ...
        if env_keys.code_target_path in environ:
            ...
            outputs = {"handler.py": py_code, "function.yaml": gen_config(config)}
    ...
    def handle_cell_magic(self, lines, io, config):
        ...
        if not handler:
            if magic.name not in magic_module.commands:
                raise NameError("unknown nuclio command: {}".format(magic.name))
            else:
...
def mount(magic, config):
    ...
    if len(rest) != 2:
        raise MagicError("2 arguments must be provided (mount point and remote path)")
